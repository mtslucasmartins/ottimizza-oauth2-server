...
public class OrganizationService {
	...
	// @formatter:off
	public OrganizationDTO findById(BigInteger id, Principal principal)
	...
	public Page<OrganizationDTO> fetchAll(OrganizationDTO filter, SearchCriteria criteria, Principal principal)
		...
		if (authenticated.getType().equals(User.Type.CUSTOMER)) {
			// results = organizationRepository.fetchAllByCustomerId(filter, pageRequest,
			// authenticated);
	...
	public List<UserDTO> fetchCustomers(User authorizedUser) {
		if (authorizedUser.getType().equals(User.Type.ACCOUNTANT)) {
			return UserDTO
					.fromEntities(userRepository.findCustomersByAccountingId(authorizedUser.getOrganization().getId()));
		}
	...
	public boolean checkIfOrganizationIsNotAlreadyRegistered(Organization organization)
		...
		if (organizationRepository.cnpjIsAlreadyRegistered(organization.getCnpj(), organization.getId(),
				accountingId)) {
	...
	public boolean checkIfOrganizationIsNotParentOfItself(Organization organization) throws IllegalArgumentException {
